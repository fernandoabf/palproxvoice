...
- **ducking de comunicação do Windows** (mic abaixava TODOS os outros sons, **mesmo com "Não fazer nada"**): a causa real é o **`echoCancellation`** do Chromium, que abre um stream de **referência de comunicação** e dispara o ducking. Corrigido **desligando o EC** (`echoCancellation:false`) + a feature `ChromeWideEchoCancellation` do WebView2 (`--disable-features`). Mantém supressão de ruído + ganho automático; EC é dispensável com fone. A chave `UserDuckingPreference=3` (gravada pelo `.exe` no HKCU do usuário) fica como reforço.
